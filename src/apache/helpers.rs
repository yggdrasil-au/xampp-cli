...
pub fn current_apache_pid(root: &Path) -> Result<Option<u32>> {
    let Some(pid) = read_pid(root)? else {
        // Fallback: scan processes for httpd.exe that matches our expected path
    ...
    if process_matches_httpd(pid, root)? {
        ...
        // PID file stale; try process scan fallback
        find_apache_pid_by_process(root)
    }
}

// Find Apache PID by scanning running processes for httpd.exe and matching its image path.
fn find_apache_pid_by_process(root: &Path) -> Result<Option<u32>> {
    ...
    for line in text.lines() {
        ...
        if l.is_empty() || l.starts_with("INFO:") {
            continue;
        }
...
fn process_matches_httpd(pid: u32, root: &Path) -> Result<bool> {
    ...
    // Try to open the process for limited information. If we can't open it
    // (due to permissions, system process, or invalid PID), treat it as not
    // matching rather than failing the whole operation.
...
fn query_process_image(handle: HANDLE) -> Result<PathBuf> {
    ...
    loop {
        ...
        let written =
            unsafe { K32GetModuleFileNameExW(Some(handle), Some(HMODULE::default()), &mut buffer) } as usize;
